...
//    @RequiresPermissionsDesc(menu={"代码字典" , "库表绑定"}, button="查询分页")
    ...
    public Object list(String oltTableDesp,
        ...
        return adminTableDespService.list(oltTableDesp,olfFieldName,olfFieldDesp, page, limit, sort);
    }
...
//    @RequiresPermissionsDesc(menu={"代码字典" , "库表绑定"}, button="详情")
    ...
    /**
     * 查询所有表和视图信息，不包括那些被限制的不可编辑表
     * @return
     */
//    @RequiresPermissions("admin:tableField:tablelist")
//    @RequiresPermissionsDesc(menu={"代码字典" , "库表绑定"}, button="查询字段")
    @GetMapping("/tableAndViewList")
    public Object gettableAndViewList() {
        List<ViewTableDef> sysTalbeDesps= adminTableDespService.getTableAndViewNotInclude();
    ...
    public Object getTableListAll() {
        List<ViewTableDef> sysTalbeDesps= adminTableDespService.getAllTable();
        return ResponseUtil.okList(sysTalbeDesps);
    }
    ...
    public Object getTableFieldList(@NotNull String tableOrViewName) {
        List<ViewTableFieldDef> viewTableFieldDefList= adminTableDespService.getAllTableFields(tableOrViewName);
